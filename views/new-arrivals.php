...
$queryParams = $_GET;
$queryParams["route"] = "new-arrivals";
    ...
    <main class="collection-page">
      <section class="collection-toolbar" aria-label="Collection controls">
        ...
        <select class="collection-toolbar__sort" aria-label="Sort by">
          ...
          <option value="newest" <?= ($sort ?? 'newest') === 'newest' ? 'selected' : '' ?>>Date, new to old</option>
      ...
      <section class="collection-grid" aria-label="New arrivals products">
        <?php if (empty($pageProducts)): ?>
          <p class="collection-grid__empty">No new arrivals at the moment.</p>
        <?php else: ?>
